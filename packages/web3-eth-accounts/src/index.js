 ...
 */
 ...
 var RLP = require('../../eth-lib/src/rlp'); // jshint ignore:line
 var Bytes = require('../../eth-lib/src/bytes'); // jshint ignore:line
 ...
 Accounts.prototype.createBLS =async function createBLS(entropy) {
     return this._addAccountFunctions(Account.createBLS(entropy || utils.randomHex(32)));
 };
 Accounts.prototype.create = function create(entropy) {
     return this._addAccountFunctions(Account.create(entropy || utils.randomHex(32)));
 };
 Accounts.prototype.privateKeyToAccountBLS = async function privateKeyToAccountBLS(privateKey, ignoreLength) {
     var account = await Account.fromPrivateBLS(privateKey)
     return this._addAccountFunctions(account);
 };
 
 Accounts.prototype.privateKeyToAccount =async function privateKeyToAccount(privateKey, ignoreLength) {
     var account = await Account.fromPrivateBLS(privateKey)
     return this._addAccountFunctions(account);
 };
 Accounts.prototype.signTransaction = function signTransaction(tx, privateKey, callback) {
     ...
     function signed(tx) {
         ...
         try {
             ...
             try{
                 var transactionHashBLS = bls_.privateToPublic(Buffer.from(privateKey, 'hex'));
                 var encoded = transactionHashBLS.s.toString("hex");
                 var transactionHashSend = "0x"+encoded;
             }catch(error){
                 console.log(error);
             }
             var serializedData = ethTx.serialize();
     ...
     return _this.privateKeyToAccount(privateKey).then((result)=>{
         return Promise.all([
             isNot(tx.gasPrice) ? _this._ethereumCall.getGasPrice() : tx.gasPrice,
             isNot(tx.nonce) ? _this._ethereumCall.getTransactionCount(result.address) : tx.nonce,
         ]).then(function (args) {
             if (isNot(args[0]) || isNot(args[1])) {
                 throw new Error('One of the values "chainId", "networkId", "gasPrice", or "nonce" couldn\'t be fetched: ' + JSON.stringify(args));
             }
             return signed(_.extend(tx, { chainId: 69, gasPrice: args[0], nonce: args[1], networkId: 1}));
         });
     });
