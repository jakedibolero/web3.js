...
const createBLS = entropy => {
  const innerHex = keccak256(Bytes.concat(Bytes.random(32), entropy || Bytes.random(32)));
  const middleHex = Bytes.concat(Bytes.concat(Bytes.random(32), innerHex), Bytes.random(32));
  const outerHex = keccak256(middleHex);
  return getRandomWalletBLS();
};
...
const getRandomWalletBLS = async() => {
  var wallet = await bls_.getVanityWalletRandom();
  return wallet;
};
const fromPrivateBLS = async privateKey => {
  var wallet = await bls_.getVanityWalletPrivate(privateKey);
  return wallet;
};
